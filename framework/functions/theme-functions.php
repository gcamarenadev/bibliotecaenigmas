<?php
   ...
   * File name:          theme-functions.php
   * Description:        This file contains the general template functions.
   * Date:               03-12-2025
   */
   ...
   * - Get Interest Posts @be_changed
   * - Get Popular posts
   * - Get popular posts @be_changed
  ...
  if (function_exists ('add_image_size')) {
    add_image_size ('tie-small', 100, 58, true);
    add_image_size ('tie-medium', 194, 112, true);
    add_image_size ('tie-large', 660, 380, true);
    
    add_image_size ('tie-library_widget', 66, 105, true); // Book widget image of the library theme
    add_image_size ('tie-library_related', 139, 218, true); // Book last posts image of the library theme
  }
  ...
  # WooCommerce Cart
  /*-----------------------------------------------------------------------------------*/
  add_filter ('woocommerce_add_to_cart_fragments', 'tie_woocommerce_add_cart_fragment', 1);
  function tie_woocommerce_add_cart_fragment ($fragments = false)
  {
    
    $output = '<span id="cart-header-count" class="shooping-count-outer">';
    
    if (WC ()->cart->get_cart_contents_count () != 0) {
      $output .= '<span class="shooping-count">' . WC ()->cart->get_cart_contents_count () . '</span>';
    }
    
    $output .= '<i class="fa fa-shopping-cart"></i></span>';
    
    $fragments['#cart-header-count'] = $output;
  ...
  # Custom Gravatar
  /*-----------------------------------------------------------------------------------*/
  function tie_custom_gravatar ($avatar = false)
  {
  ...
  # Exclude pages From Search
  /*-----------------------------------------------------------------------------------*/
  function tie_search_filter ($query = false)
  {
  ...
  function tie_author_box ($avatar = true, $social = true, $name = false, $user_id = false)
  {
    ...
    <div class="author-social flat-social">
      <?php if (get_the_author_meta ('url', $user_id)) : ?>
        <a class="social-site" target="_blank" href="<?php echo esc_url (get_the_author_meta ('url', $user_id)); ?>"><i
              class="fa fa-home"></i></a>
      <?php endif ?>
      <?php if (get_the_author_meta ('facebook', $user_id)) : ?>
        <a class="social-facebook" target="_blank"
           href="<?php echo esc_url (get_the_author_meta ('facebook', $user_id)); ?>"><i class="fa fa-facebook"></i></a>
      <?php endif ?>
      <?php if (get_the_author_meta ('twitter', $user_id)) : ?>
        <a class="social-twitter" target="_blank"
           href="http://twitter.com/<?php the_author_meta ('twitter', $user_id); ?>"><i class="fa fa-twitter"></i><span> @<?php the_author_meta ('twitter', $user_id); ?></span></a>
      <?php endif ?>
      <?php if (get_the_author_meta ('linkedin', $user_id)) : ?>
        <a class="social-linkedin" target="_blank"
           href="<?php echo esc_url (get_the_author_meta ('linkedin', $user_id)); ?>"><i class="fa fa-linkedin"></i></a>
      <?php endif ?>
      <?php if (get_the_author_meta ('flickr', $user_id)) : ?>
        <a class="social-flickr" target="_blank"
           href="<?php echo esc_url (get_the_author_meta ('flickr', $user_id)); ?>"><i class="fa fa-flickr"></i></a>
      <?php endif ?>
      <?php if (get_the_author_meta ('youtube', $user_id)) : ?>
        <a class="social-youtube" target="_blank"
           href="<?php echo esc_url (get_the_author_meta ('youtube', $user_id)); ?>"><i class="fa fa-youtube"></i></a>
      <?php endif ?>
      <?php if (get_the_author_meta ('pinterest', $user_id)) : ?>
        <a class="social-pinterest" target="_blank"
           href="<?php echo esc_url (get_the_author_meta ('pinterest', $user_id)); ?>"><i
              class="fa fa-pinterest"></i></a>
      <?php endif ?>
      <?php if (get_the_author_meta ('behance', $user_id)) : ?>
        <a class="social-behance" target="_blank"
           href="<?php echo esc_url (get_the_author_meta ('behance', $user_id)); ?>"><i class="fa fa-behance"></i></a>
      <?php endif ?>
      <?php if (get_the_author_meta ('instagram', $user_id)) : ?>
        <a class="social-instagram" target="_blank"
           href="<?php echo esc_url (get_the_author_meta ('instagram', $user_id)); ?>"><i
    ...
    <div class="social-icons<?php echo $colored ?>">
      <?php
        ...
        if (tie_get_option ('rss_icon')) {
          if (tie_get_option ('rss_url') != '' && tie_get_option ('rss_url') != ' ')
            $rss = tie_get_option ('rss_url');
          else $rss = get_bloginfo ('rss2_url');
          ?><a class="<?php echo $tooltip; ?>" title="Rss" href="<?php echo esc_url ($rss); ?>" <?php echo $newtab; ?>>
            <i class="fa fa-rss"></i></a><?php
        }
        // Facebook
        if (!empty($facebook) && $facebook != ' ') {
          ?><a class="<?php echo $tooltip; ?>" title="Facebook"
               href="<?php echo esc_url ($facebook); ?>" <?php echo $newtab; ?>><i class="fa fa-facebook"></i></a><?php
        }
        // Twitter
        if (!empty($twitter) && $twitter != ' ') {
          ?><a class="<?php echo $tooltip; ?>" title="Twitter"
               href="<?php echo esc_url ($twitter); ?>" <?php echo $newtab; ?>><i class="fa fa-twitter"></i></a><?php
        }
        // Pinterest
        if (!empty($Pinterest) && $Pinterest != ' ') {
          ?><a class="<?php echo $tooltip; ?>" title="Pinterest"
               href="<?php echo esc_url ($Pinterest); ?>" <?php echo $newtab; ?>><i class="fa fa-pinterest"></i>
          </a><?php
        }
        // dribbble
        if (!empty($dribbble) && $dribbble != ' ') {
          ?><a class="<?php echo $tooltip; ?>" title="Dribbble"
               href="<?php echo esc_url ($dribbble); ?>" <?php echo $newtab; ?>><i class="fa fa-dribbble"></i></a><?php
        }
        // LinkedIN
        if (!empty($linkedin) && $linkedin != ' ') {
          ?><a class="<?php echo $tooltip; ?>" title="LinkedIn"
               href="<?php echo esc_url ($linkedin); ?>" <?php echo $newtab; ?>><i class="fa fa-linkedin"></i></a><?php
        }
        // evernote
        if (!empty($evernote) && $evernote != ' ') {
          ?><a class="<?php echo $tooltip; ?>" title="Evernote"
               href="<?php echo esc_url ($evernote); ?>" <?php echo $newtab; ?>><i class="tieicon-evernote"></i>
          </a><?php
        }
        // Flickr
        if (!empty($flickr) && $flickr != ' ') {
          ?><a class="<?php echo $tooltip; ?>" title="Flickr"
               href="<?php echo esc_url ($flickr); ?>" <?php echo $newtab; ?>><i class="tieicon-flickr"></i></a><?php
        }
        // Picasa
        if (!empty($picasa) && $picasa != ' ') {
          ?><a class="<?php echo $tooltip; ?>" title="Picasa"
               href="<?php echo esc_url ($picasa); ?>" <?php echo $newtab; ?>><i class="tieicon-picasa"></i></a><?php
        }
        // YouTube
        if (!empty($youtube) && $youtube != ' ') {
          ?><a class="<?php echo $tooltip; ?>" title="Youtube"
               href="<?php echo esc_url ($youtube); ?>" <?php echo $newtab; ?>><i class="fa fa-youtube"></i></a><?php
        }
        // Skype
        if (!empty($skype) && $skype != ' ') {
          ?><a class="<?php echo $tooltip; ?>" title="Skype"
               href="<?php echo esc_url ($skype); ?>" <?php echo $newtab; ?>><i class="fa fa-skype"></i></a><?php
        }
        // Digg
        if (!empty($digg) && $digg != ' ') {
          ?><a class="<?php echo $tooltip; ?>" title="Digg"
               href="<?php echo esc_url ($digg); ?>" <?php echo $newtab; ?>><i class="fa fa-digg"></i></a><?php
        }
        // Reddit
        if (!empty($reddit) && $reddit != ' ') {
          ?><a class="<?php echo $tooltip; ?>" title="Reddit"
               href="<?php echo esc_url ($reddit); ?>" <?php echo $newtab; ?>><i class="fa fa-reddit"></i></a><?php
        }
        // Delicious
        if (!empty($delicious) && $delicious != ' ') {
          ?><a class="<?php echo $tooltip; ?>" title="Delicious"
               href="<?php echo esc_url ($delicious); ?>" <?php echo $newtab; ?>><i class="fa fa-delicious"></i>
          </a><?php
        }
        // stumbleuponUpon
        if (!empty($stumbleupon) && $stumbleupon != ' ') {
          ?><a class="<?php echo $tooltip; ?>" title="StumbleUpon"
               href="<?php echo esc_url ($stumbleupon); ?>" <?php echo $newtab; ?>><i class="fa fa-stumbleupon"></i>
          </a><?php
        }
        // Tumblr
        if (!empty($tumblr) && $tumblr != ' ') {
          ?><a class="<?php echo $tooltip; ?>" title="Tumblr"
               href="<?php echo esc_url ($tumblr); ?>" <?php echo $newtab; ?>><i class="fa fa-tumblr"></i></a><?php
        }
        // Vimeo
        if (!empty($vimeo) && $vimeo != ' ') {
          ?><a class="<?php echo $tooltip; ?>" title="Vimeo"
               href="<?php echo esc_url ($vimeo); ?>" <?php echo $newtab; ?>><i class="tieicon-vimeo"></i></a><?php
        }
        // Blogger
        if (!empty($blogger) && $blogger != ' ') {
          ?><a class="<?php echo $tooltip; ?>" title="Blogger"
               href="<?php echo esc_url ($blogger); ?>" <?php echo $newtab; ?>><i class="tieicon-blogger"></i></a><?php
        }
        // Wordpress
        if (!empty($wordpress) && $wordpress != ' ') {
          ?><a class="<?php echo $tooltip; ?>" title="WordPress"
               href="<?php echo esc_url ($wordpress); ?>" <?php echo $newtab; ?>><i class="fa fa-wordpress"></i>
          </a><?php
        }
        // Yelp
        if (!empty($yelp) && $yelp != ' ') {
          ?><a class="<?php echo $tooltip; ?>" title="Yelp"
               href="<?php echo esc_url ($yelp); ?>" <?php echo $newtab; ?>><i class="fa fa-yelp"></i></a><?php
        }
        // Last.fm
        if (!empty($lastfm) && $lastfm != ' ') {
          ?><a class="<?php echo $tooltip; ?>" title="Last.fm"
               href="<?php echo esc_url ($lastfm); ?>" <?php echo $newtab; ?>><i class="fa fa-lastfm"></i></a><?php
        }
        // grooveshark
        if (!empty($grooveshark) && $grooveshark != ' ') {
          ?><a class="<?php echo $tooltip; ?>" title="Grooveshark"
               href="<?php echo esc_url ($grooveshark); ?>" <?php echo $newtab; ?>><i class="tieicon-grooveshark"></i>
          </a><?php
        }
        // sharethis
        if (!empty($sharethis) && $sharethis != ' ') {
          ?><a class="<?php echo $tooltip; ?>" title="ShareThis"
               href="<?php echo esc_url ($sharethis); ?>" <?php echo $newtab; ?>><i class="fa fa-share-alt"></i>
          </a><?php
        }
        // dropbox
        if (!empty($dropbox) && $dropbox != ' ') {
          ?><a class="<?php echo $tooltip; ?>" title="Dropbox"
               href="<?php echo esc_url ($dropbox); ?>" <?php echo $newtab; ?>><i class="fa fa-dropbox"></i></a><?php
        }
        // xing.me
        if (!empty($xing) && $xing != ' ') {
          ?><a class="<?php echo $tooltip; ?>" title="Xing"
               href="<?php echo esc_url ($xing); ?>" <?php echo $newtab; ?>><i class="fa fa-xing"></i></a><?php
        }
        // DeviantArt
        if (!empty($deviantart) && $deviantart != ' ') {
          ?><a class="<?php echo $tooltip; ?>" title="DeviantArt"
               href="<?php echo esc_url ($deviantart); ?>" <?php echo $newtab; ?>><i class="tieicon-deviantart"></i>
          </a><?php
        }
        // Apple
        if (!empty($apple) && $apple != ' ') {
          ?><a class="<?php echo $tooltip; ?>" title="Apple"
               href="<?php echo esc_url ($apple); ?>" <?php echo $newtab; ?>><i class="fa fa-apple"></i></a><?php
        }
        // foursquare
        if (!empty($foursquare) && $foursquare != ' ') {
          ?><a class="<?php echo $tooltip; ?>" title="Foursquare"
               href="<?php echo esc_url ($foursquare); ?>" <?php echo $newtab; ?>><i class="fa fa-foursquare"></i>
          </a><?php
        }
        // github
        if (!empty($github) && $github != ' ') {
          ?><a class="<?php echo $tooltip; ?>" title="Github"
               href="<?php echo esc_url ($github); ?>" <?php echo $newtab; ?>><i class="fa fa-github"></i></a><?php
        }
        // soundcloud
        if (!empty($soundcloud) && $soundcloud != ' ') {
          ?><a class="<?php echo $tooltip; ?>" title="SoundCloud"
               href="<?php echo esc_url ($soundcloud); ?>" <?php echo $newtab; ?>><i class="fa fa-soundcloud"></i>
          </a><?php
        }
        // behance
        if (!empty($behance) && $behance != '' && $behance != ' ') {
          ?><a class="<?php echo $tooltip; ?>" title="Behance"
               href="<?php echo esc_url ($behance); ?>" <?php echo $newtab; ?>><i class="fa fa-behance"></i></a><?php
        }
        // instagram
        if (!empty($instagram) && $instagram != '' && $instagram != ' ') {
          ?><a class="<?php echo $tooltip; ?>" title="instagram"
               href="<?php echo esc_url ($instagram); ?>" <?php echo $newtab; ?>><i class="fa fa-instagram"></i>
          </a><?php
        }
        // paypal
        if (!empty($paypal) && $paypal != '' && $paypal != ' ') {
          ?><a class="<?php echo $tooltip; ?>" title="paypal"
               href="<?php echo esc_url ($paypal); ?>" <?php echo $newtab; ?>><i class="fa fa-paypal"></i></a><?php
        }
        // spotify
        if (!empty($spotify) && $spotify != '' && $spotify != ' ') {
          ?><a class="<?php echo $tooltip; ?>" title="spotify"
               href="<?php echo esc_url ($spotify); ?>" <?php echo $newtab; ?>><i class="fa fa-spotify"></i></a><?php
        }
        // viadeo
        if (!empty($viadeo) && $viadeo != '' && $viadeo != ' ') {
          ?><a class="<?php echo $tooltip; ?>" title="viadeo"
               href="<?php echo esc_url ($viadeo); ?>" <?php echo $newtab; ?>><i class="tieicon-viadeo"></i></a><?php
        }
        // Google Play
        if (!empty($google_play) && $google_play != '' && $google_play != ' ') {
          ?><a class="<?php echo $tooltip; ?>" title="Google Play"
               href="<?php echo esc_url ($google_play); ?>" <?php echo $newtab; ?>><i class="fa fa-play"></i></a><?php
        }
        // 500PX
        if (!empty($px500) && $px500 != ' ') {
          ?><a class="<?php echo $tooltip; ?>" title="500px"
               href="<?php echo esc_url ($px500); ?>" <?php echo $newtab; ?>><i class="tieicon-fivehundredpx"></i>
          </a><?php
        }
        // Forrst
        if (!empty($forrst) && $forrst != ' ') {
          ?><a class="<?php echo $tooltip; ?>" title="Forrst"
               href="<?php echo esc_url ($forrst); ?>" <?php echo $newtab; ?>><i class="tieicon-forrst"></i></a><?php
        }
        // VK
        if (!empty($vk) && $vk != ' ') {
          ?><a class="<?php echo $tooltip; ?>" title="vk.com"
               href="<?php echo esc_url ($vk); ?>" <?php echo $newtab; ?>><i class="fa fa-vk"></i></a><?php
        }
        // SnapChat
        if (!empty($snapchat) && $snapchat != ' ') {
          ?><a class="<?php echo $tooltip; ?>" title="Snapchat"
               href="<?php echo esc_url ($snapchat); ?>" <?php echo $newtab; ?>><i class="fa fa-snapchat"></i></a><?php
        }
      ...
      <?php //Custom Social Networking
        ...
        for ($i = 1; $i <= 5; $i++) {
          if (tie_get_option ("custom_social_icon_$i") && tie_get_option ("custom_social_url_$i")) {
            ?><a class="<?php echo $tooltip; ?>" <?php if (tie_get_option ("custom_social_title_$i"))
              echo ' title="' . tie_get_option ("custom_social_title_$i") . '"'; ?>
                 href="<?php echo esc_url (tie_get_option ('custom_social_url_' . $i)) ?>" <?php echo $newtab; ?>><i
                class="fa <?php echo tie_get_option ("custom_social_icon_$i") ?>"></i></a><?php
          }
        }
  ...
  # Change The Default WordPress Excerpt Length
  /*-----------------------------------------------------------------------------------*/
  function tie_excerpt_global_length ($length = false)
  {
  ...
  function tie_excerpt_home_length ($length = false)
  {
    global $get_meta;
  ...
  # Read More Functions
  /*-----------------------------------------------------------------------------------*/
  function tie_remove_excerpt ($more = false)
  {
    return ' &hellip;';
  }
  ...
  # Get Video embed URL
  /*-----------------------------------------------------------------------------------*/
  function tie_get_video_embed ($video_url = false)
  {
    $protocol = is_ssl () ? 'https' : 'http';
    $video_link = @parse_url ($video_url);
    if ($video_link['host'] == 'www.youtube.com' || $video_link['host'] == 'youtube.com') {
  ...
  # Tie Excerpt
  /*-----------------------------------------------------------------------------------*/
  function tie_content_limit ($text = false, $chars = 120)
  {
  ...
  # tie Thumb
  /*-----------------------------------------------------------------------------------*/
  function tie_slider_img_src ($image_id = false, $size = false)
  {
    global $post;
    $image_url = wp_get_attachment_image_src ($image_id, $size);
    return !empty($image_url[0]) ? $image_url[0] : false;
  ...
  function tie_show_extra_profile_fields ($user = false)
  {
    wp_enqueue_media ();
    ?>
    ...
    <table class="form-table">
      <tr>
        ...
        <td>
          ...
          <input id="author-cover-bg" class="img-path" type="text" size="56" style="direction:ltr; text-laign:left"
                 name="author-cover-bg" value="<?php if (!empty($author_cover_bg))
            echo esc_attr ($author_cover_bg); ?>"/>
          <input id="upload_author-cover-bg_button" type="button" class="button" value="<?php _e ('Upload', 'tie') ?>"/>

          <div id="author-cover-bg-preview" class="img-preview" <?php if (empty($author_cover_bg))
            echo 'style="display:none;"' ?>>
            <img src="<?php if (!empty($author_cover_bg))
              echo $author_cover_bg; else echo get_template_directory_uri () . '/framework/admin/images/empty.png'; ?>"
                 alt=""/>
    ...
    <table class="form-table">
      <tr>
        ...
        <td>
          <textarea name="author_widget_content" id="author_widget_content" rows="5"
                    cols="30"><?php echo esc_attr (get_the_author_meta ('author_widget_content', $user->ID)); ?></textarea>
      ...
      <tr>
        ...
        <td>
          <input type="text" name="facebook" id="facebook"
                 value="<?php echo esc_url (get_the_author_meta ('facebook', $user->ID)); ?>"
                 class="regular-text"/><br/>
        </td>
      </tr>
      <tr>
        ...
        <td>
          <input type="text" name="linkedin" id="linkedin"
                 value="<?php echo esc_url (get_the_author_meta ('linkedin', $user->ID)); ?>"
                 class="regular-text"/><br/>
        </td>
      </tr>
      <tr>
        ...
        <td>
          <input type="text" name="flickr" id="flickr"
                 value="<?php echo esc_url (get_the_author_meta ('flickr', $user->ID)); ?>" class="regular-text"/><br/>
        </td>
      </tr>
      <tr>
        ...
        <td>
          <input type="text" name="youtube" id="youtube"
                 value="<?php echo esc_url (get_the_author_meta ('youtube', $user->ID)); ?>" class="regular-text"/><br/>
        </td>
      </tr>
      <tr>
        ...
        <td>
          <input type="text" name="pinterest" id="pinterest"
                 value="<?php echo esc_url (get_the_author_meta ('pinterest', $user->ID)); ?>"
                 class="regular-text"/><br/>
        </td>
      </tr>
      <tr>
        ...
        <td>
          <input type="text" name="behance" id="behance"
                 value="<?php echo esc_url (get_the_author_meta ('behance', $user->ID)); ?>" class="regular-text"/><br/>
        </td>
      </tr>
      <tr>
        ...
        <td>
          <input type="text" name="instagram" id="instagram"
                 value="<?php echo esc_url (get_the_author_meta ('instagram', $user->ID)); ?>"
                 class="regular-text"/><br/>
        </td>
      </tr>
  ...
  function tie_save_extra_profile_fields ($user_id = false)
  {
  ...
  # Get Feeds
  /*-----------------------------------------------------------------------------------*/
  function tie_get_feeds ($feed = false, $number = 10)
  {
  ...
  function tie_wp_footer ()
  {
    if (tie_get_option ('footer_code'))
      echo htmlspecialchars_decode (stripslashes (tie_get_option ('footer_code')));
    
    //Reading Position Indicator
    if (tie_get_option ('reading_indicator') && is_singular ())
  ...
  # News In Picture
  /*-----------------------------------------------------------------------------------*/
  function tie_last_news_pic ($order = false, $posts_number = 12, $cats = 1)
  {
    global $post;
    $original_post = $post;
      ...
      while ($get_posts_query->have_posts ()) : $get_posts_query->the_post () ?>
        <?php if (function_exists ("has_post_thumbnail") && has_post_thumbnail ()) : ?>
          <div <?php tie_post_class ('post-thumbnail'); ?>>
            <a class="ttip" title="<?php the_title (); ?>"
               href="<?php the_permalink (); ?>"><?php the_post_thumbnail ('tie-small'); ?><span
    ...
    if ($get_posts_query->have_posts ()):
      while ($get_posts_query->have_posts ()) : $get_posts_query->the_post () ?>
        ...
        <li style=" padding: 0; border-bottom: none;">
          ...
          <!-- Title /-->
          <span class="post-views-widget widget-post-title">
          ...
          <!--/Categories-->
          <span class="post-views-widget widget-post-categories">
            <i class="fa-solid fa-arrow-down-a-z"></i>
            <?php printf ('%1$s', get_the_category_list (', ')); ?>
          </span>

          <!-- Date /-->
          <span class="post-views-widget widget-post-date">
              <?php tie_get_time (); ?>
          </span>
          ...
          <!-- Views /-->
          <span class="post-views-widget mt0">
    ...
    if ($get_posts_query->have_posts ()):
      while ($get_posts_query->have_posts ()) : $get_posts_query->the_post () ?>
        <li <?php tie_post_class (); ?>>
          <?php if (function_exists ("has_post_thumbnail") && has_post_thumbnail () && $thumb) : ?>
            <div class="post-thumbnail">
              <a href="<?php the_permalink (); ?>" rel="bookmark"><?php the_post_thumbnail ('tie-small'); ?><span
                    class="fa overlay-icon"></span></a>
            </div><!-- post-thumbnail /-->
          <?php endif; ?>
          ...
          <?php if (function_exists ("has_post_thumbnail") && has_post_thumbnail () && $thumb) : ?>
            <div class="post-thumbnail">
              <a href="<?php echo get_author_posts_url (get_the_author_meta ('ID')) ?>"
                 title=""><?php echo get_avatar (get_the_author_meta ('user_email'), apply_filters ('MFW_author_bio_avatar_size', 50)); ?></a>
            </div><!-- post-thumbnail /-->
          <?php endif; ?>
          <h3><a href="<?php the_permalink (); ?>"><?php the_title (); ?></a></h3>
          <strong><a href="<?php echo get_author_posts_url (get_the_author_meta ('ID')) ?>"
                     title=""><?php echo get_the_author () ?> </a></strong>
        </li>
      <?php
      endwhile;
    endif;
      ...
      while ($popularposts->have_posts ()) : $popularposts->the_post () ?>
        <li <?php tie_post_class (); ?>>
          ...
          <?php if (get_comments_number () != 0) : ?>
            <span class="post-comments post-comments-widget"><i
                  class="fa fa-comments"></i><?php comments_popup_link ('0', '1', '%'); ?></span>
          <?php endif; ?>
        </li>
      <?php
      endwhile;
  ...
  # Google Map Function
  /*-----------------------------------------------------------------------------------*/
  function tie_google_maps ($src = false, $width = 610, $height = 440, $class = "")
  {
  ...
  # Soundcloud Function
  /*-----------------------------------------------------------------------------------*/
  function tie_soundcloud ($url = false, $autoplay = 'false', $visual = 'false')
  {
    global $post;
    ...
    if (empty($tie_post_color) && tie_get_option ('theme_skin') && !tie_get_option ('global_color'))
      $tie_post_color = tie_get_option ('theme_skin');
    if (empty($tie_post_color) && tie_get_option ('global_color'))
      $tie_post_color = tie_get_option ('global_color');
    ...
    <?php else: ?>
      <div id="login-form">
        <form name="loginform" id="loginform" action="<?php echo esc_url (site_url ('wp-login.php', 'login_post')) ?>"
              method="post">
          <p id="log-username"><input type="text" name="log" id="log" title="<?php _eti ('Username') ?>"
                                      value="<?php _eti ('Username') ?>"
                                      onfocus="if (this.value == '<?php _eti ('Username') ?>') {this.value = '';}"
                                      onblur="if (this.value == '') {this.value = '<?php _eti ('Username') ?>';}"
                                      size="33"/></p>
          <p id="log-pass"><input type="password" name="pwd" id="pwd" title="<?php _eti ('Password') ?>"
                                  value="<?php _eti ('Password') ?>"
                                  onfocus="if (this.value == '<?php _eti ('Password') ?>') {this.value = '';}"
                                  onblur="if (this.value == '') {this.value = '<?php _eti ('Password') ?>';}"
                                  size="33"/></p>
          ...
          <label for="rememberme"><input name="rememberme" id="rememberme" type="checkbox" checked="checked"
                                         value="forever"/> <?php _eti ('Remember Me') ?></label>
  ...
  # For Empty Widgets Titles
  /*-----------------------------------------------------------------------------------*/
  function tie_widget_title ($title = false)
  {
    if (empty($title))
      return ' ';
    else return $title;
  ...
  function tie_body_custom_class ($classes = false)
  {
  ...
  # Fix Shortcodes
  /*-----------------------------------------------------------------------------------*/
  function tie_fix_shortcodes ($content = false)
  {
    $array = array(
      '[raw]' => '',
      '[/raw]' => '',
  ...
  function tie_video_fix_shortcodes ($content = false)
  {
    $v = '/(\[(video)\s?.*?\])(.+?)(\[(\/video)\])/';
  ...
  # Custom Comments Template
  /*-----------------------------------------------------------------------------------*/
  function tie_custom_comments ($comment = false, $args = false, $depth = false)
  {
      ...
      <div class="comment-content">
        <div class="author-comment">
          ...
          <div class="comment-meta commentmetadata"><a
                href="<?php echo esc_url (get_comment_link ($comment->comment_ID)); ?>">  <?php printf (__ti ('%1$s at %2$s'), get_comment_date (), get_comment_time ()); ?></a><?php edit_comment_link (__ti ('Edit'), ' '); ?>
  ...
  # Custom Pings Template
  /*-----------------------------------------------------------------------------------*/
  function tie_custom_pings ($comment = false, $args = false, $depth = false)
  {
    $GLOBALS['comment'] = $comment; ?>
  ...
  function tie_theme_register_required_plugins ()
  {
    ...
    if (!get_option ('tie_token_' . SAHIFA_ID) && !empty($_SERVER['SERVER_NAME']) && strpos ($_SERVER['SERVER_NAME'], 'tielabs.com') === false) {
      return;
    }
    ...
    $plugins = array(
      ...
      array(
        'name' => 'Arqam Lite',
        'slug' => 'arqam-lite',
        'source' => get_template_directory_uri () . '/framework/plugins/arqam-lite.zip',
        'required' => true,
        'version' => '1.0.11',
...
*/
  ...
  function tie_remove_tgm_notice ($val = false, $object_id = false, $meta_key = false, $single = false)
  {
  ...
  # BANNERS
  /*-----------------------------------------------------------------------------------*/
  function tie_banner ($banner = false, $before = false, $after = false)
  {
    if (tie_get_option ($banner)):
      ...
      if (tie_get_option ($banner . '_img')):
        ...
        if (tie_get_option ($banner . '_nofollow'))
          $nofollow = 'rel="nofollow"'; ?>

        <a href="<?php echo tie_get_option ($banner . '_url') ?>"
           title="<?php echo tie_get_option ($banner . '_alt') ?>" <?php echo $target;
          echo $nofollow ?>>
          <img src="<?php echo tie_get_option ($banner . '_img') ?>"
               alt="<?php echo tie_get_option ($banner . '_alt') ?>"/>
        </a>
      <?php
  ...
  # Remove Query Strings From Static Resources
  /*-----------------------------------------------------------------------------------*/
  function tie_remove_query_strings_1 ($src = false)
  {
    $rqs = explode ('?ver', $src);
    return $rqs[0];
  ...
  function tie_remove_query_strings_2 ($src = false)
  {
    $rqs = explode ('&ver', $src);
    return $rqs[0];
  }
  ...
  # Sanitizes a title, replacing whitespace and a few other characters with dashes.
  /*-----------------------------------------------------------------------------------*/
  function tie_sanitize_title ($title = false)
  {
  ...
  # get terms as plain text seprated with commas
  /*-----------------------------------------------------------------------------------*/
  function tie_get_plain_terms ($post_id = false, $term = false)
  {
  ...
  # Remove hentry
  /*-----------------------------------------------------------------------------------*/
  function tie_remove_hentry_from_post_class_filter ($classes = false)
  {
  ...
  if (!function_exists ('tie_article_schemas')) {
    ...
    function tie_article_schemas ()
    {
      ...
      $schema_type = tie_get_option ('schema_type');
      $schema_type = !empty($schema_type) ? $schema_type : 'Article';
      $schema_type = ($has_review) ? 'review' : $schema_type;
      
      $schema = array(
        '@context' => 'http://schema.org',
        '@type' => $schema_type, // CreativeWork | NewsArticle | Article | BlogPosting
      ...
      if (!empty($has_review)) {
        ...
        if (function_exists ('taqyeem_review_get_rich_snippet')) {
      ...
      echo '<script type="application/ld+json" class="tie-schema-graph">' . json_encode ($schema) . '</script>';
    }
  }
   ...
   * License Validation message
   */
  add_filter ('wp_footer', 'sahifa_validation_message');
  function sahifa_validation_message ()
  {
    
    if (!get_option ('tie_token_' . SAHIFA_ID) && !empty($_SERVER['SERVER_NAME']) && strpos ($_SERVER['SERVER_NAME'], 'tielabs.com') === false) {
      ?>
      <div class="theme-notice"
           style="padding: 20px 0;font-size: 14px;z-index:9999999!important;position:fixed!important;width:100%!important;bottom:0!important;display:block!important;text-align:center;background:#f44336!important;color:#fff!important;">
        <a style="color: #fff; text-decoration: underline;"
           href="https://tielabs.com/buy/sahifa?utm_source=footer-bar&utm_medium=link&utm_campaign=sahifa">Sahifa
          Theme</a> License is not validated, Go to the theme options page to validate the license, You need a single
        license for each domain name.
      </div>
      <?php
    }
  }